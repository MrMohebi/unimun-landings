...
const Carousel = () => {
    ...
    useEffect(() => {
        ...
        // let items = document.querySelector(".book-image-container");
        // gsap.to(carolRef.current, {
        //     scrollTrigger: {}
        // });
    ...
    return (
        <>
            <div dir={"ltr"} className={"relative w-full lg:fixed lg:bottom-64 lg:w-full translate-x-32 "}
                 ref={carolRef}>
                    ...
                    style={{scrollBehavior: "initial"}}>
                    {
                        Array(40).fill(0).map((item, index) => {
                            let div = <motion.div style={{opacity: "0"}} id={"book-" + index} key={index}
                                ...
                                <img id={"img-" + index} className={"rounded-2xl w-28"} style={{height: "100%"}}
                                     ...
                                     alt="book"/>
                                <span className={"shrink-0 mt-1"}>رایگان {index}</span>
                            </motion.div>;
